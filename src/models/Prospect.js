...
const prospectSchema = new mongoose.Schema(
  {
    ...
    // ROI Calculation (from Voiceflow or frontend)
    calculatedROI: {
...
prospectSchema.methods.calculateLeadScore = function () {
  ...
  // Higher score for more enquiries
  if (this.monthlyEnquiries > 500) score += 30;
  else if (this.monthlyEnquiries > 200) score += 20;
  else if (this.monthlyEnquiries > 100) score += 10;

  // Bonus for using Reapit (easier integration)
  if (this.currentCRM === 'reapit') score += 15;

  // Pain points indicate urgency
  if (this.painPoints?.includes('after_hours')) score += 15;
  if (this.painPoints?.includes('response_time')) score += 10;

  // Team size indicates budget
  if (this.teamSize > 5) score += 10;

  // Multiple branches = bigger opportunity
  if (this.numberOfBranches > 1) score += 10;

  return Math.min(score, 100); // Cap at 100
};

// Method to check if prospect is qualified
prospectSchema.methods.isQualified = function () {
  return (
    this.monthlyEnquiries >= 50 && this.email && this.phone && this.companyName
  );
};
